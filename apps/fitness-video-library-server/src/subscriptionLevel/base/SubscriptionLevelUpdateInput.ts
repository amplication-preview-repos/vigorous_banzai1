/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { InputType, Field } from "@nestjs/graphql";
import { ApiProperty } from "@nestjs/swagger";
import { IsString, IsOptional, ValidateNested } from "class-validator";
import { SubscriptionUpdateManyWithoutSubscriptionLevelsInput } from "./SubscriptionUpdateManyWithoutSubscriptionLevelsInput";
import { Type } from "class-transformer";
import { VideoUpdateManyWithoutSubscriptionLevelsInput } from "./VideoUpdateManyWithoutSubscriptionLevelsInput";

@InputType()
class SubscriptionLevelUpdateInput {
  @ApiProperty({
    required: false,
    type: String,
  })
  @IsString()
  @IsOptional()
  @Field(() => String, {
    nullable: true,
  })
  description?: string | null;

  @ApiProperty({
    required: false,
    type: String,
  })
  @IsString()
  @IsOptional()
  @Field(() => String, {
    nullable: true,
  })
  name?: string | null;

  @ApiProperty({
    required: false,
    type: () => SubscriptionUpdateManyWithoutSubscriptionLevelsInput,
  })
  @ValidateNested()
  @Type(() => SubscriptionUpdateManyWithoutSubscriptionLevelsInput)
  @IsOptional()
  @Field(() => SubscriptionUpdateManyWithoutSubscriptionLevelsInput, {
    nullable: true,
  })
  subscriptions?: SubscriptionUpdateManyWithoutSubscriptionLevelsInput;

  @ApiProperty({
    required: false,
    type: () => VideoUpdateManyWithoutSubscriptionLevelsInput,
  })
  @ValidateNested()
  @Type(() => VideoUpdateManyWithoutSubscriptionLevelsInput)
  @IsOptional()
  @Field(() => VideoUpdateManyWithoutSubscriptionLevelsInput, {
    nullable: true,
  })
  videos?: VideoUpdateManyWithoutSubscriptionLevelsInput;
}

export { SubscriptionLevelUpdateInput as SubscriptionLevelUpdateInput };
