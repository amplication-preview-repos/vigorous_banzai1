/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { InputType, Field } from "@nestjs/graphql";
import { ApiProperty } from "@nestjs/swagger";
import { IsString, IsOptional, ValidateNested } from "class-validator";
import { SubscriptionCreateNestedManyWithoutSubscriptionLevelsInput } from "./SubscriptionCreateNestedManyWithoutSubscriptionLevelsInput";
import { Type } from "class-transformer";
import { VideoCreateNestedManyWithoutSubscriptionLevelsInput } from "./VideoCreateNestedManyWithoutSubscriptionLevelsInput";

@InputType()
class SubscriptionLevelCreateInput {
  @ApiProperty({
    required: false,
    type: String,
  })
  @IsString()
  @IsOptional()
  @Field(() => String, {
    nullable: true,
  })
  description?: string | null;

  @ApiProperty({
    required: false,
    type: String,
  })
  @IsString()
  @IsOptional()
  @Field(() => String, {
    nullable: true,
  })
  name?: string | null;

  @ApiProperty({
    required: false,
    type: () => SubscriptionCreateNestedManyWithoutSubscriptionLevelsInput,
  })
  @ValidateNested()
  @Type(() => SubscriptionCreateNestedManyWithoutSubscriptionLevelsInput)
  @IsOptional()
  @Field(() => SubscriptionCreateNestedManyWithoutSubscriptionLevelsInput, {
    nullable: true,
  })
  subscriptions?: SubscriptionCreateNestedManyWithoutSubscriptionLevelsInput;

  @ApiProperty({
    required: false,
    type: () => VideoCreateNestedManyWithoutSubscriptionLevelsInput,
  })
  @ValidateNested()
  @Type(() => VideoCreateNestedManyWithoutSubscriptionLevelsInput)
  @IsOptional()
  @Field(() => VideoCreateNestedManyWithoutSubscriptionLevelsInput, {
    nullable: true,
  })
  videos?: VideoCreateNestedManyWithoutSubscriptionLevelsInput;
}

export { SubscriptionLevelCreateInput as SubscriptionLevelCreateInput };
